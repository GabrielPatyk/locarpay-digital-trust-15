...
const ConfiguracoesInquilino = () => {
  ...
  const confirmPersonalChanges = async () => {
    ...
    if (formData.cpf.trim()) {
      updateData.cpf = unformatCNPJ(formData.cpf);
    }
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <Input
                  ...
                  placeholder={user?.cpf ? formatCNPJ(user.cpf) : "000.000.000-00"}
                />
                {user?.cpf && (
                  <p className="text-xs text-gray-500 mt-1">Atual: {formatCNPJ(user.cpf)}</p>
                )}
              </div>
            </div>
