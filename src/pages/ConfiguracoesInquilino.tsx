...
const ConfiguracoesInquilino = () => {
  ...
  const { formatPhone, formatCPF, unformatPhone, unformatCPF, isValidPhone } = usePhoneFormatter();
  ...
  const handleInputChange = (field: string, value: string | boolean) => {
    if (field === 'telefone') {
      ...
      setFormData(prev => ({
        ...prev,
        [field]: formatCPF(value as string)
      }));
    } else {
      setFormData(prev => ({
        ...prev,
  ...
  const confirmPersonalChanges = async () => {
    ...
    if (formData.cpf.trim()) updateData.cpf = unformatCPF(formData.cpf);

    // Dados do perfil
    if (formData.endereco.trim()) profileData.endereco = formData.endereco;
    if (formData.numero.trim()) profileData.numero = formData.numero;
    if (formData.complemento.trim()) profileData.complemento = formData.complemento;
    if (formData.bairro.trim()) profileData.bairro = formData.bairro;
    if (formData.cidade.trim()) profileData.cidade = formData.cidade;
    if (formData.estado.trim()) profileData.estado = formData.estado;
    if (formData.pais.trim()) profileData.pais = formData.pais;

    let success = true;

    // Atualizar dados do usuário
    if (Object.keys(updateData).length > 0) {
      success = await updateUserData(updateData);
    }

    // Atualizar perfil
    if (success && Object.keys(profileData).length > 0) {
          ...
          <CardContent className="space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <Input
                  ...
                  placeholder={user?.cpf ? formatCPF(user.cpf) : "000.000.000-00"}
                />
                {user?.cpf && (
                  <p className="text-xs text-gray-500 mt-1">Atual: {formatCPF(user.cpf)}</p>
                )}
              </div>
            </div>
            ...
            <div>
              ...
              <Input
                ...
                placeholder={profile?.complemento || "Apartamento, Bloco, etc."}
              />
              {profile?.complemento && (
